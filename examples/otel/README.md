...
### 2.1. KV-cache invalidation between sessions

By default, the replayer preserves the original message content from traces, which can lead to
KV-cache hits across different sessions if they share similar prefixes. To ensure clean KV-cache
state between sessions (useful for measuring worst-case performance or testing cache-miss scenarios),
enable `inject_random_session_id`:
...
data:
  ...
  otel_trace_replay:
    ...
    inject_random_session_id: false               # inject random string into unique segments
                                                  # to invalidate KV-cache between sessions

    # Session duplication
    duplicate_sessions_target: null               # optional: set to a number for total count of sessions after duplication
                                                  # useful for generating more load from limited traces
