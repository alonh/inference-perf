    ...
    while cursor < total_msgs:
        ...
        for pred_idx, pred in enumerate(predecessors):
            ...
            for msg_idx in range(cursor, total_msgs):
                ...
                matched = output_matches_message(pred_out, msg)
                # Fallback: match by tool-call IDs when text representations differ
                # (e.g. OTel parts format vs OpenAI tool_calls format in input messages).
                # Uses the pre-computed ID set to avoid repeated extraction.
                if not matched and pred_tool_ids:
                    matched = bool(pred_tool_ids <= msg_tool_id_sets[msg_idx])  # all pred IDs present in msg
                if matched:
